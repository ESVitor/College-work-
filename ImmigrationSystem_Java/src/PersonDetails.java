...
public class PersonDetails {
	...
	public void addToTheList() {
		person = new Person(fn, ln, doa, pn);

		switch (hk) {
		case 0:
			linkedList.addNewPerson(person);
			break;
		case 1:
			linkedList.addNewPersonToTheFirstPosition(person);
			break;
		case 2:
			linkedList.addNewPersonToTheMidleOfTheList(person);
			break;
		case 3:
			linkedList.addNewPersonAnyIndex(position, person);
		}
			...
			try {
				...
				if (linkedList.isEmpty() == true) {
					...
					if(linkedList.size()>= indexes){
						//continue
						linkedList.removeNPersonFromTheEndOfTheList(indexes);
						inputOk = true;
					}else{
						System.out.println("There is only "+linkedList.size()+" positions on the list");
					}
